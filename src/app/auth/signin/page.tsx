...
const SignIn: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (token) {
      router.replace("/employee/time-entry");
    }
  }, [router]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const tokenCheckResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/auth/token-check`, {
          headers: {
        ...
        if (tokenCheckResponse.ok) {
          const userData = await tokenCheckResponse.json();
  ...
  return (
    <div className="flex min-h-screen content-center rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
